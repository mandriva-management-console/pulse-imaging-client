...
void check_bootsect(FILE * f, PARAMS * p, CPARAMS * cp) {
    char *str, dum[512];
    long i;
    ...
    cp->FirstDataSector =
        cp->bootsect.BPB_RsvdSecCnt +
        (cp->bootsect.BPB_NumFATs * cp->FAT_size) + cp->RootDirSectors;
    ...
    debug
        ("- Nb of Clusters    : %ld (x<4085=FAT12,x<65525=FAT16, else FAT32)\n",
         i =
         ((p->nb_sect - cp->FirstDataSector) / cp->bootsect.BPB_SecPerClus));
