...
void setbit(unsigned char *ptr, unsigned long nb) {
    unsigned char mask[8] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80 };

    ptr[nb >> 3] |= mask[nb & 7];
}

void parse_datarun(unsigned char *ptr, FILE * file, PARAMS * p, CPARAMS * cp) {
    int n1, n2, i, j, k, l;
    ...
    do {
        ...
        if (fseeko
            (file, ((unsigned long long)offset) * cp->ClustSize, SEEK_SET)) {
...
void parse_mft(unsigned char *ptr, FILE * file, PARAMS * p, CPARAMS * cp) {
    ...
    while ((ptr - origptr < 1024)
           && (at_type = (*(unsigned long *)ptr)) != 0xFFFFFFFF) {
        nptr = parse_attr(ptr);
        switch (at_type) {
        case 0x10:
        ...
        case 0x70:
            ...
            debug(" Volume Information : v%d.%d flags:%04x\n", *(ptr + 8),
                  *(ptr + 9), *(unsigned short *)(ptr + 10));
            if (*(unsigned short *)(ptr + 10) != 0) {
                if (*(unsigned short *)(ptr + 10) == 0x4000) {
                    debug
                        (" Strange volume flags == 0x4000, so I continue...\n");
                } else {
...
void get_bitmap(FILE * fi, PARAMS * p, CPARAMS * cp) {
...
void get_volume(FILE * fi, PARAMS * p, CPARAMS * cp) {
    unsigned long long pos;
    char mft_entry[1024];
 ...
 */
void comp_mft(FILE * fi, PARAMS * p, CPARAMS * cp) {
...
void parse_boot(FILE * fi, PARAMS * p, CPARAMS * cp) {
    int i;
    unsigned char *ptr;
    __u32 size;
    ...
    if (ioctl(fileno(fi), BLKGETSIZE, &size) < 0) {
        ...
        if (cp->boot.sectors > size) {
            debug
                ("Filesystem size > Block dev size: probably part of a RAID volume.\nERROR: Cannot backup NTFS RAID volumes.\n");
            exit(1);
        }
    }
...
void count_used(PARAMS * p, CPARAMS * cp) {
