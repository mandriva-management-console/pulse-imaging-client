...
int identify_func(char *arg, int flags) {
    ...
    if (strstr(arg, "L=fr_FR")) {
        title_prompt = "\n\
͵  Déclaration d'un poste client au serveur Pulse Imaging  \n\
\n\
    Lors de la déclaration du poste, si l'identifiant respecte le format\n\
    suivant :\n\
\n\
        <profil>:/<entité_A>/<entité_B>/<nom-de-l'ordinateur>\n\
\n\
    Le poste sera directement ajouté au profil <profil> et à l'entité\n\
    /entité_A/entité_B.\n\
\n\
...
\n\
    Atenéao: Quando possivel, use o nome da maquina com ID.\
";
